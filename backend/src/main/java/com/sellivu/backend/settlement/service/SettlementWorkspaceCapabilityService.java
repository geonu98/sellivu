...
public class SettlementWorkspaceCapabilityService {
    ...
    public AnalysisCapabilityResponse getCapability(String workspaceKey, String workspaceToken) {
        SettlementWorkspace workspace = validateWorkspace(workspaceKey, workspaceToken);
        ...
        for (SettlementWorkspaceFile file : files) {
            if (file.getFileType() != null) {
                uploadedTypes.add(file.getFileType());
            }
        }
    ...
    private SettlementWorkspace validateWorkspace(String workspaceKey, String workspaceToken) {
        ...
        if (workspace.isExpired()) {
            throw new ApiException(ErrorCode.WORKSPACE_EXPIRED);
        }
    ...
    private String buildMessage(Set<SettlementFileType> uploadedTypes) {
        ...
        if (!hasDaily && !hasOrder && !hasFee) {
            return "이 워크스페이스에는 업로드된 정산 파일이 없습니다.";
        }

        if (hasDaily && !hasOrder && !hasFee) {
            return "이 워크스페이스에서는 일별 정산 파일만 포함되어 일별/월별 분석만 가능합니다.";
        }

        if (!hasDaily && hasOrder && !hasFee) {
            return "이 워크스페이스에서는 건별 정산 파일만 포함되어 건별 정산 조회만 가능합니다.";
        }

        if (!hasDaily && !hasOrder && hasFee) {
            return "이 워크스페이스에서는 수수료 상세 파일만 포함되어 수수료 상세 조회만 가능합니다.";
        }

        if (hasDaily && hasOrder && !hasFee) {
            return "이 워크스페이스에서는 일별 정산과 건별 정산 파일이 포함되어 일별/월별/건별 분석이 가능하며, 수수료 상세 파일이 없어 수수료 교차검증은 제한됩니다.";
        }

        if (hasDaily && !hasOrder && hasFee) {
            return "이 워크스페이스에서는 일별 정산과 수수료 상세 파일이 포함되어 일별/월별 및 수수료 상세 조회가 가능하며, 건별 정산 파일이 없어 건별 교차분석은 제한됩니다.";
        }

        if (!hasDaily && hasOrder && hasFee) {
            return "이 워크스페이스에서는 건별 정산과 수수료 상세 파일이 포함되어 건별 교차검증이 가능합니다. 일별 정산 파일이 없어 일별 합계 검증은 제한됩니다.";
        }

        return "이 워크스페이스에는 일별 정산, 건별 정산, 수수료 상세 파일이 모두 포함되어 전체 분석과 검증이 가능하며, 정책성 차이에 대한 추가 옵션 입력으로 판정 정확도를 높일 수 있습니다.";
    }
}