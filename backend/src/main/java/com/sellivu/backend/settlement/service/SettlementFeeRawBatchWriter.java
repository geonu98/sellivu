...
public class SettlementFeeRawBatchWriter {
    ...
    public int write(Long runId, List<SettlementFeeRow> rows) {
        ...
        StringBuilder sb = new StringBuilder(Math.max(1024, rows.size() * 200));
        ...
        for (SettlementFeeRow row : rows) {
            ...
            append(sb, row.getFeeBaseAmount());
            append(sb, row.getFeeType());
            append(sb, row.getPaymentMethod());
            append(sb, row.getSalesLinkedFeeDetail());
            append(sb, row.getFeeCapAmount());
            append(sb, row.getCommissionAmount());
            appendLast(sb, now);
        }
        ...
        try (InputStream inputStream = new ByteArrayInputStream(sb.toString().getBytes(StandardCharsets.UTF_8))) {
            ...
            throw new RuntimeException("settlement_fee_raw COPY insert 실패", e);
        } finally {
